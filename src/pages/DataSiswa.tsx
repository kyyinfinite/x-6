...
import { siswaList as siswaListDummy } from "../DataDummy";
...
export default function DataSiswa() {
  ...
  const { data, error } = usePolling(() => apiGet<Siswa[]>("/api/students"), 10000);
  const siswaList = data && data.length > 0 ? data : siswaListDummy;
    ...
    <div className="space-y-6 p-6">
      {error && (
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/10 text-amber-300 text-xs px-4 py-2.5">
          ⚠️ Gagal memuat data siswa dari server ({error}), menampilkan data contoh sementara.
        </div>
      )}
