...
export default function DataSiswa() {
  ...
  const avgNilai = siswaList.length
    ...
    : "0.0";
  const topSiswa = [...siswaList].sort((a, b) => b.nilaiRata - a.nilaiRata)[0];
  ...
  return (
    <div className="space-y-6 p-6">
      {error && (
        <div className="rounded-xl border border-red-500/20 bg-red-500/10 text-red-300 text-xs px-4 py-2.5">
      ...
      {/* Empty state */}
      {siswaList.length === 0 ? (
        <div className="glass rounded-2xl border border-white/5 flex flex-col items-center justify-center py-20 text-slate-500">
          <Users size={48} className="mb-4 opacity-30" />
          <p className="text-sm font-medium">Belum ada data siswa</p>
          <p className="text-xs text-slate-600 mt-1">Data akan muncul setelah siswa ditambahkan</p>
        </div>
      ) : (
        <>
          {/* Top Student */}
