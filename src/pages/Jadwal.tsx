...
import { BookOpen, Clock, MapPin, User, CalendarOff, ClipboardList } from "lucide-react";
...
export default function Jadwal() {
  ...
  } = usePolling<Record<string, JadwalItem[]>>(
    () => apiGet<Record<string, JadwalItem[]>>("/api/schedule"),
    30000
  );
  ...
  const {
    data: jadwalPiket,
    error: piketError,
    loading: piketLoading,
  } = usePolling<PiketItem[]>(
    () => apiGet<PiketItem[]>("/api/schedule?type=piket"),
    30000
  );
    ...
    <div className="space-y-6 p-6">
      ...
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">
          Jadwal pelajaran dan piket harian kelas X-6
        </p>
      </div>

      {/* Tabs */}
      {(jadwalLoading || piketLoading) ? (
        ...
        <>
              ...
              <div className="p-4 border-b border-white/5">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-white text-sm">
                    Jadwal {selectedHari} - Kelas X-6
                  </h3>
                </div>
              </div>
              <div className="p-12 flex flex-col items-center justify-center text-center gap-3">
                <div className="p-3 rounded-2xl bg-white/5">
                  <CalendarOff size={28} className="text-slate-500" />
                </div>
                <p className="text-slate-400 text-sm font-medium">
                  Tidak ada jadwal untuk hari ini
                </p>
                <p className="text-slate-600 text-xs">
                  Jadwal pelajaran untuk {selectedHari} belum tersedia.
                </p>
              </div>
            </div>
          ) : (
              ...
              <div className="p-4 space-y-2">
                {jadwal.map((j, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={`flex items-stretch gap-3 rounded-xl border px-4 py-3 transition-all ${
                        isBreak
                          ? "opacity-50"
                          : "hover:bg-white/3"
                      } ${
                        MAPEL_COLORS[j.mapel] ||
                    ...
                    >
                      <div className="flex flex-col items-center">
                        <div
                          ...
                          style={{
                            background: isBreak ? "#475569" : "#3b82f6",
                          }}
                        />
                        {i < jadwal.length - 1 && (
                          <div
          ...
          {jadwalLoading ? (
            <div className="glass rounded-2xl p-5 border border-white/5">
              <div className="animate-pulse bg-white/[0.06] rounded-lg h-4 w-44 mb-4" />
              <div className="flex flex-wrap gap-2">
                {Array.from({ length: 6 }).map((_, i) => (
                  <div
                    key={i}
                    className="animate-pulse bg-white/[0.06] rounded-lg h-7 w-24"
                  />
                ))}
              </div>
            </div>
                ...
                {jadwal
                  ...
                  .map((j, i) => (
                    <span
                      ...
                      } ${MAPEL_TEXT[j.mapel] || "text-slate-300"}`}
                    >
                      {j.mapel}
                    </span>
                  ))}
        ...
        (piketLoading ? (
          ...
          <div className="glass rounded-2xl border border-white/5 overflow-hidden">
            <div className="p-12 flex flex-col items-center justify-center text-center gap-3">
              ...
              <p className="text-slate-600 text-xs">
                Jadwal piket belum tersedia saat ini.
              </p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {jadwalPiket.map((p, pi) => {
              const colors = [
                "blue",
                "purple",
                "green",
              ...
              const colorMap: Record<string, string> = {
                ...
                purple:
                  "border-purple-500/20 from-purple-500/8 text-purple-400 bg-purple-500/10",
                green:
                  "border-green-500/20 from-green-500/8 text-green-400 bg-green-500/10",
                amber:
                  "border-amber-500/20 from-amber-500/8 text-amber-400 bg-amber-500/10",
              ...
              return (
                <div
                  key={pi}
                  className={`glass rounded-2xl p-5 border bg-gradient-to-br ${cls.split(" ")[0]} ${cls.split(" ")[1]}`}
                >
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="font-bold text-white">{p.hari}</h3>
                    <span
                      className={`badge ${cls.split(" ")[2]} ${cls.split(" ")[3]}`}
                    >
                      Kelompok {p.kelompok}
                    </span>
                  </div>
                  <div className="space-y-2.5">
                    {p.anggota.map((nama, j) => (
                      <div
                        key={j}
                        className="flex items-center gap-3 bg-white/3 rounded-xl p-2.5"
                      >
                        <img
