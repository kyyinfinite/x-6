...
import { jadwalPelajaran as jadwalPelajaranDummy, jadwalPiket as jadwalPiketDummy } from "../DataDummy";
...
export default function Jadwal() {
  ...
  const { data: liveJadwal, error: jadwalError } = usePolling(
    () => apiGet<typeof jadwalPelajaranDummy>("/api/schedule"),
    30000
  );
  ...
  const jadwalPelajaran = liveJadwal && Object.keys(liveJadwal).length > 0 ? liveJadwal : jadwalPelajaranDummy;
  const jadwalPiket = livePiket && livePiket.length > 0 ? livePiket : jadwalPiketDummy;
    ...
    <div className="space-y-6 p-6">
      {error && (
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/10 text-amber-300 text-xs px-4 py-2.5">
          ⚠️ Gagal memuat jadwal dari server ({error}), menampilkan data contoh sementara.
        </div>
      )}
      <div>
