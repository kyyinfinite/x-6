...
export default function Nilai() {
  const { data: nilaiMapel, error: gradesError, loading: gradesLoading } =
    usePolling<NilaiMapelItem[]>(() => apiGet<NilaiMapelItem[]>("/api/grades"));

  const { data: siswaList, error: siswaError, loading: siswaLoading } =
    usePolling<Siswa[]>(() => apiGet<Siswa[]>("/api/students"));
  ...
  const top5 = siswaList
    ? [...siswaList].sort((a, b) => b.nilaiRata - a.nilaiRata).slice(0, 5)
    : [];

  const avgKelas =
    nilaiMapel && nilaiMapel.length
      ? (
          nilaiMapel.reduce((a, n) => a + n.rataRata, 0) / nilaiMapel.length
        ).toFixed(1)
      : "0.0";

  const best = nilaiMapel?.length
    ? nilaiMapel.reduce((a, b) => (a.rataRata > b.rataRata ? a : b))
    : null;

  const worst = nilaiMapel?.length
    ? nilaiMapel.reduce((a, b) => (a.rataRata < b.rataRata ? a : b))
    : null;

  const hasGrades = nilaiMapel && nilaiMapel.length > 0;
  const hasSiswa = siswaList && siswaList.length > 0;
    ...
    <div className="space-y-6 p-6">
      ...
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">
          Nilai rata-rata kelas X-6 per mata pelajaran
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="glass rounded-2xl p-5 border border-white/5 flex items-center justify-center min-h-[120px]"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="glass rounded-2xl p-5 border border-blue-500/15 bg-gradient-to-br from-blue-500/8 to-transparent">
            ...
            <p className="text-xs text-slate-500 mt-2">
              Dari {nilaiMapel.length} mata pelajaran
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {loading ? (
          ...
          <div className="glass rounded-2xl p-5 border border-white/5 flex items-center justify-center min-h-[340px]">
            <p className="text-sm text-slate-500">Belum ada data nilai</p>
          </div>
        ) : (
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      style={{
                        background: `${medals[i]}22`,
                        color: medals[i],
                        border: `1px solid ${medals[i]}40`,
                      }}
                    >
                      {i + 1}
                    </div>
              ...
              <thead>
                <tr className="border-b border-white/5">
                  {[
                    ...
                    <th
                      key={h}
                      className="text-left p-4 text-xs text-slate-500 uppercase tracking-wider whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
                ...
                {nilaiMapel.map((n, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4">
                        <span
                          className={`badge border ${gradeColor}`}
                        >
                          {grade}
                        </span>
                      </td>
