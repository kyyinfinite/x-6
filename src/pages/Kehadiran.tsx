...
import { kehadiranBulanan as kehadiranBulananDummy, siswaList as siswaListDummy, kehadiranHariIni as kehadiranHariIniDummy } from "../DataDummy";
...
type Siswa = (typeof siswaListDummy)[number];
...
export default function Kehadiran() {
  ...
  const kehadiranBulanan = liveBulanan ?? kehadiranBulananDummy;
  ...
  }, [siswaList.length, kehadiranHariIni.tanggal]);
            ...
            {siswaList.map((s) => {
              ...
              return (
                <div key={s.nama} className="flex items-center gap-3 bg-white/3 rounded-xl p-3 border border-white/3">
                  <span className="text-xs text-slate-600 w-5 text-center font-mono">{s.id}</span>
